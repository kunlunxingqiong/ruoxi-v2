...
class InkBleedEffect:
    ...
    def apply_ink_bleed(self, text: str, emotion_intensity: float) -> str:
        """应用墨迹扩散效果"""
        # 根据情感强度调整扩散程度
        self.bleed_radius = min(
            self.max_radius,
            5 + emotion_intensity * 10
        )
...
class MouseFollower:
    ...
    def calculate_offset(self, mouse_pos: Tuple[float, float], 
                         emotional_state: Dict) -> Tuple[float, float]:
        """计算文本偏移量"""
        # 根据情感状态调整跟随行为
    ...
    def apply_emotion_style(self, text: str, emotion: str) -> Dict:
        ...
        if style.get("glow"):
            ...
            result["effects"].append({
                "type": "glow",
                "color": "#ffeaa7",
                "intensity": style["glow"]
            })
        
        return result


def render_ruoxi_text(text: str, 
                     emotion: str = "tender",
    ...
    # 添加墨迹效果
    ink_result = ink.apply_ink_bleed(text, emotional_state.get("intensity", 0.5) if emotional_state else 0.5)
