...
class DataImporter:
    ...
    # 列名映射 (标准列名 -> 可能的变体)
    COLUMN_MAPPINGS = {
        "timestamp": ["timestamp", "date", "time", "datetime", "记录时间", "日期", "时间"],
    ...
    async def import_data(
        ...
        # 导入到数据库
        imported_count, import_errors = await self._store_data(
            df, data_type, user_id
        )
    ...
    def _clean_data(
        ...
        if data_type == DataType.BLOOD_PRESSURE:
            ...
            # 过滤异常值
            invalid = df[(df['systolic'] < 50) | (df['systolic'] > 250) |
                        (df['diastolic'] < 30) | (df['diastolic'] > 150)]
            
            if not invalid.empty:
                warnings.append(f"发现 {len(invalid)} 行异常血压值")
                df = df[(df['systolic'] >= 50) & (df['systolic'] <= 250) &
                       (df['diastolic'] >= 30) & (df['diastolic'] <= 150)]
