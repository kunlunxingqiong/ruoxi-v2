...
class DataExporter:
    ...
    async def export_health_report(
        ...
        if export_format == ExportFormat.PDF:
            ...
            return await self.export(
                ...
                template=(
                    ExportTemplate.WEEKLY_REPORT
                    if report_type == "weekly"
                    else ExportTemplate.MONTHLY_REPORT
                ),
            )
    ...
    def _apply_template(
        self, df: pd.DataFrame, template: ExportTemplate, data_type: str
    ...
    def _generate_file(
        self, df: pd.DataFrame, export_format: ExportFormat, include_charts: bool
    ) -> tuple[bytes, str, str]:
        """生成文件内容"""
    ...
    def _generate_excel(
        self, df: pd.DataFrame, include_charts: bool
    ) -> tuple[bytes, str, str]:
        """生成Excel文件"""
        output = BytesIO()
    ...
    async def send_by_email(
        self, export_result: ExportResult, email: str, subject: Optional[str] = None
    ) -> bool:
        """
        通过邮件发送导出文件
