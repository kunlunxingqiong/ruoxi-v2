...
class ConversationManager:
    ...
    def build_optimized_context(
        self, session: ConversationSession, current_query: str, use_memory: bool = True
    ) -> List[Dict[str, str]]:
        """
        构建优化的对话上下文
        ...
        if len(session.turns) > 3:
            related_turns = self._find_related_turns(
                session, current_query, max_results=2
            )
            for turn in related_turns:
                # 避免重复添加
                if not any(t.get("content") == turn.user_message for t in recent_turns):
                    messages.append(
                        {"role": "user", "content": f"[之前] {turn.user_message}"}
                    )
                    messages.append(
                        {
                            "role": "assistant",
                            "content": f"[之前回复] {turn.assistant_response}",
                        }
                    )
                    total_length += len(turn.user_message) + len(
        ...
        if use_memory:
            memories = memory_manager.retrieve_relevant_memories(
                query=current_query, user_id=session.user_id, top_k=2
            )
    ...
    def _build_system_prompt(
        self, session: ConversationSession, current_query: str
    ) -> str:
        """构建系统提示词"""
...
"""
    ...
    def _find_related_turns(
        self, session: ConversationSession, query: str, max_results: int = 2
        ...
        for turn in (
            session.turns[:-6] if len(session.turns) > 6 else session.turns
        ):  # 排除最近的
            # 计算话题相似度
    ...
    def save_conversation_turn(
        ...
        **metadata,
        ...
        memory_manager.add_to_context(
            session_id, user_id, "assistant", assistant_response
        )
...
if __name__ == "__main__":
    ...
    ctx = manager.build_optimized_context(
        manager.get_or_create_session(session_id, user_id), "你好若曦"
    )
    print(f"\n第1轮上下文长度: {len(ctx)} 条消息")
    manager.save_conversation_turn(
        session_id, user_id, "你好若曦", "🌸 你好呀~ 曦曦在哦"
    )

    # 第二轮
    ctx = manager.build_optimized_context(
        manager.get_or_create_session(session_id, user_id), "我今天有点头疼"
    )
    print(f"第2轮上下文长度: {len(ctx)} 条消息")
    manager.save_conversation_turn(
        session_id, user_id, "我今天有点头疼", "抱抱你... 头疼的话可以先休息一下"
    )

    # 第三轮 ( referencing previous )
    ctx = manager.build_optimized_context(
        manager.get_or_create_session(session_id, user_id), "刚才说的休息，具体怎么做？"
    )
    print(f"第3轮上下文长度: {len(ctx)} 条消息")
    print(
        f"  包含的历史引用: {[m.get('content', '')[:30] for m in ctx if '[之前]' in m.get('content', '')]}"
    )
