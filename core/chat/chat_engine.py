    ...
    async def _extract_sources(
        ...
        for mem in memories:
            sources.append({
                ...
                "timestamp": mem.created_at.isoformat() if hasattr(mem, 'created_at') else None
            })
...
class ChatService:
    ...
    async def send_message(
        ...
        """发送消息"""
        chat_mode = ChatMode(mode) if mode in [m.value for m in ChatMode] else ChatMode.CASUAL
