...
class DashscopeClient(BaseModel):
    """
    阿里百炼 API 客户端
    ...
    def __init__(
        ...
        if not self._api_key:
            raise ValueError(
                "Dashscope API key not found. Set DASHSCOPE_API_KEY environment variable."
            )

        super().__init__(self._api_key, model_name, ModelProvider.DASHSCOPE)
    ...
    async def chat(
        ...
        **kwargs
        ...
        if not await self._check_rate_limit():
            raise Exception("Dashscope rate limit exceeded")
