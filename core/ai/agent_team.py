...
class AgentTeam:
    ...
    def _auto_match_role(self, task: str) -> AgentRole:
        ...
        # 健康相关 → 阿芙
        health_kw = ["健康", "血压", "血糖", "体检", "医疗", "疾病", "症状", "用药",
                      "吃药", "失眠", "头疼", "不舒服", "感冒", "发烧"]
        if any(kw in task_lower for kw in health_kw):
            return AgentRole.AFU
        
        # 代码相关 → 小码
        code_kw = ["代码", "编程", "函数", "bug", "debug", "程序", "算法",
                    "sql", "python", "javascript", "编译", "报错", "运行错误"]
        if any(kw in task_lower for kw in code_kw):
            return AgentRole.CODER
        
        # 调研相关 → 小研
        research_kw = ["调研", "研究", "分析报告", "调查", "文献", "资料",
                        "长文本", "总结", "论文", "学术"]
        ...
        if combine_method == "parallel":
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    processed.append(TaskResult(role=roles[i], content="", success=False, error=str(result)))
                else:
