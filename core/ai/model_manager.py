...
class ModelManager:
        ...
        try:
            ...
            # 新Provider
                ...
                if config.provider == ModelProvider.NVIDIA:
                    model = ClientClass(
                        config.api_key, config.model_name, config.api_key_2
                    )
                elif config.provider == ModelProvider.CLOUDFLARE:
                    model = ClientClass(
                        config.api_key, config.model_name, config.account_id or ""
                    )
                else:
                    model = ClientClass(config.api_key, config.model_name)
    ...
    async def generate(
        self, messages: Union[List[Message], List[Dict]], **kwargs
    ) -> ModelResponse:
        """
    ...
    def _select_model_by_role(self, agent_role: str) -> Optional[str]:
        """根据角色选择模型"""
        try:
            from core.ai.router.route_config import ROLE_DEFAULT_MODELS, AgentRole
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "providers": list(set(c.provider.value for c in self._configs.values())),
        }
    ...
    async def discover_all_models(self):
        ...
        """
        try:
            from core.ai.router.model_registry import DiscoveredModel, model_registry
        except ImportError:
        ...
        if discovered:
            ...
            logger.info(
                f"动态发现总计: {len(discovered)} 个模型，注册表总计: {model_registry.total_count}"
            )
