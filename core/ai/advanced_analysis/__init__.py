...
class AdvancedHealthAnalyzer:
    ...
    def _generate_comprehensive_recommendations(
        self, predictions: Dict, anomalies: Dict, risks: Dict
    ) -> Dict[str, Any]:
        """生成综合建议"""
        immediate_actions = []
        short_term_goals = []
        ...
        for anomaly in anomalies.get("detected_anomalies", []):
            if anomaly.get("severity") == "critical":
                immediate_actions.append(
                    f"[{anomaly['metric']}] {anomaly['recommendation']}"
                )
            elif anomaly.get("severity") == "warning":
                short_term_goals.append(
                    f"改善{anomaly['metric']}: {anomaly['recommendation']}"
                )
    ...
    def _generate_monitoring_plan(self, anomalies: Dict, risks: Dict) -> Dict[str, Any]:
        ...
        bp_frequency = (
            "daily"
            if any(
                a.get("metric") == "blood_pressure"
                and a.get("severity") in ["critical", "warning"]
                for a in anomalies.get("detected_anomalies", [])
            )
            else "weekly"
        )
