...
class HealthAnomalyDetector:
    ...
    def _detect_bp_anomalies(self, days: int) -> None:
        ...
        records = self.db.query(BloodPressureRecord).filter(
            ...
            BloodPressureRecord.measured_at >= datetime.utcnow() - timedelta(days=days)
        ).all()
        ...
        for record in recent_records:
            ...
            if record.systolic >= 180 or record.diastolic >= 120:
                self.anomalies.append(AnomalyResult(
                    ...
                    deviation_percentage=((record.systolic - 140) / 140 * 100) if record.systolic > 140 else 0,
                    affected_records=1,
        ...
        if len(weights) >= 14:
            ...
            if recent_std > 2:
                self.anomalies.append(AnomalyResult(
                    ...
                    current_value=recent_weight,
                    expected_range=(mean(weights[-14:]) - 1, mean(weights[-14:]) + 1),
    ...
    def _zscore_outliers(self, values: List[float], threshold: float = 2.0) -> List[int]:
        """
        使用Z-score检测异常值
