    ...
    async def stream_chat_completion(
        ...
        try:
            ...
            yield StreamChunk(
                content="",
                is_start=True,
                metadata={"model": model_config.get("model", "default") if model_config else "default"}
            )
            
            # 获取AI回复 (非流式模拟，真实实现需接入流式API)
            response = await ai_manager.generate(
                messages=messages,
                stream=False  # 当前模拟，实际应启用流式
            )
...
class TokenStreamBuffer:
    ...
    async def add(self, token: str) -> Optional[str]:
        ...
        should_flush = (
            ...
            (datetime.utcnow() - self.last_flush_time).total_seconds() * 1000 >= self.flush_interval_ms
        )
