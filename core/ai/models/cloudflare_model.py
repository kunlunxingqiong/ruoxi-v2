...
class CloudflareModel(BaseModel):
    ...
    async def chat(
        ...
        if response.status_code != 200:
            raise Exception(f"Cloudflare API错误 {response.status_code}: {response.text[:200]}")
    ...
    def discover_models(cls, api_key: str, base_url: Optional[str] = None) -> List[Dict]:
        ...
        return [
            ...
            {"id": "@cf/meta/llama-3.2-1b-instruct", "name": "Llama 3.2 1B", "provider": "cloudflare"},
            {"id": "@cf/meta/llama-3.2-3b-instruct", "name": "Llama 3.2 3B", "provider": "cloudflare"},
            {"id": "@cf/mistral/mistral-7b-instruct", "name": "Mistral 7B", "provider": "cloudflare"},
            {"id": "@cf/deepseek-ai/deepseek-math-7b-instruct", "name": "DeepSeek Math 7B", "provider": "cloudflare"},
            {"id": "@cf/deepseek-ai/deepseek-coder-6.7b-instruct", "name": "DeepSeek Coder 6.7B", "provider": "cloudflare"},
        ]
