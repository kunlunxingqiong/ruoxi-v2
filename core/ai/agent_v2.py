...
from core.ai.model_manager import ModelResponse, ai_manager
from core.emotion.emotion_analyzer import EmotionType, emotion_analyzer
...
class RuoxiAgentV2:
    ...
    async def respond(
        ...
        memories = await self.memory.query_memories(
            user_id=user_id, query=message, limit=3
        )
...
"""
    ...
    async def _generate_follow_up(self, intent: str, emotion: EmotionType) -> List[str]:
        """生成后续问题建议"""
        follow_ups = {
            "健康咨询": [
                "最近还有其他不舒服的地方吗？",
