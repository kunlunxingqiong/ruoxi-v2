...
class ModelConfig:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "rate_limit": {"rpm": self.rate_limit_rpm, "rpd": self.rate_limit_rpd},
        }
...
class AIModelManager:
    ...
    def _select_best_model(
        self, capability: Optional[ModelCapability] = None, streaming: bool = False
    ) -> str:
        """
        选择最适合的模型
        ...
        if streaming:
            candidates = [
                m for m in candidates if ModelCapability.STREAMING in m.capabilities
            ]
    ...
    def _get_fallback_model(self, current_model: str) -> Optional[str]:
        ...
        all_models = [
            m
            for m in self.models.values()
            if m.model_id != current_model and m.model_id in self.clients
        ]
