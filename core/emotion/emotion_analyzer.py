...
class EmotionAnalyzer:
    ...
    EMOTION_KEYWORDS = {
        EmotionType.HAPPY: [
            "开心", "高兴", "愉快", "快乐", "兴奋", "欢喜", "爽", "棒", "好极了",
            "哈哈", "嘿嘿", "嘻嘻", "太好了", "赞", "开心死了", "好开心"
        ],
        EmotionType.SAD: [
            "难过", "伤心", "悲伤", "哭", "想哭", "失落", "郁闷", "down", "不开心",
            "难受", "痛苦", "委屈", "寂寞", "空虚", "沮丧", "绝望", "心累"
        ],
        EmotionType.ANGRY: [
            "生气", "愤怒", "发火", "气死", "烦死了", "讨厌", "恨", "不爽",
            "滚", "烦人", "恶心", "气死我了", "火大", "烦躁", "暴躁"
        ],
        EmotionType.ANXIOUS: [
            "担心", "焦虑", "紧张", "害怕", "慌", "不安", "忐忑", "怕", "恐惧",
            "着急", "心急", "压力", "喘不过气", "心慌", "焦虑死了"
        ],
        EmotionType.LONELY: [
            "孤独", "寂寞", "没人理解", "孤单", "一个人", "没人关心", "被忽视",
            "孤立", "疏离", "关系淡", "没朋友", "没人理我"
        ],
        EmotionType.TIRED: [
            "累", "疲惫", "困", "没精神", "乏力", "不想动", " exhaustion",
            "倦怠", "精疲力尽", "累死了", "好累", "撑不住了"
        ],
        EmotionType.WORRIED: [
            "担心", "挂念", "惦记", "不放心", "忧虑", "忧愁", "发愁", "操心",
            "惦记", "不放心", "担忧", "在乎", "怕出什么事"
        ],
        EmotionType.EXCITED: [
            "激动", "期待", "憧憬", "迫不及待", "兴奋", "high", "雀跃",
            "兴奋不已", "激动人心", "太期待了", "等不及"
        ]
    }
    ...
    CRISIS_KEYWORDS = {
        CrisisLevel.SEVERE: [
            "不想活", "活着没意思", "死了算了", "想自杀", "结束一切", "了结",
            "没有希望", "看不到未来", "解脱", "离开这个世界"
        ],
        CrisisLevel.MODERATE: [
            "好累", "撑不下去", "想放弃", "很痛苦", "受不了", "快疯了",
            "天天失眠", "情绪低落很久", "不想见人"
        ],
        CrisisLevel.MILD: [
            "压力大", "迷茫", "困惑", "不知所措", "需要帮助", "想聊聊",
            "最近不开心", "有点难过"
        ]
    }
    ...
    def analyze(self, text: str, user_id: str = "anonymous") -> EmotionState:
        ...
        normalized_secondary = [
            (e, min(1.0, s / 3)) for e, s in secondary
        ]
        ...
        if len(self.user_emotion_history[user_id]) > 100:
            self.user_emotion_history[user_id] = self.user_emotion_history[user_id][-100:]
    ...
    def generate_response_strategy(
        ...
        # 获取响应模板
        template = self.RESPONSE_TEMPLATES.get(emotion, {
            "style": "neutral",
            "tone": "friendly",
    ...
    def get_emotion_trend(self, user_id: str, days: int = 7) -> Dict:
        ...
        # 趋势判断
        if emotion_counts.get(EmotionType.SAD, 0) + emotion_counts.get(EmotionType.ANXIOUS, 0) > len(history) / 3:
            trend = "concerning"
        ...
        return {
            ...
            "crisis_flags": sum(1 for h in history if h.crisis_level != CrisisLevel.NONE)
        }
...
if __name__ == "__main__":
    ...
    for text in test_cases:
        ...
        print(f"\n输入: \"{text}\"")
        print(f"  主要情绪: {state.primary_emotion.value} (强度: {state.intensity:.1f})")
