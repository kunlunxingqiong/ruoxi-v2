...
class DashboardService:
    ...
    def get_today_summary(self) -> Dict[str, Any]:
        ...
        sleep_today = (
            ...
            .filter(SleepRecord.user_id == self.user_id, SleepRecord.date == today)
            .first()
        )
        ...
        # 目标打卡
        goal_checkins = (
            self.db.query(GoalCheckIn).filter(GoalCheckIn.date == today).count()
        )
    ...
    def get_weekly_stats(self) -> Dict[str, Any]:
        ...
        avg_systolic = (
            sum(r.systolic for r in bp_records) / len(bp_records)
            if bp_records
            else None
        )
        avg_diastolic = (
            sum(r.diastolic for r in bp_records) / len(bp_records)
            if bp_records
            else None
        )
        ...
        avg_glucose = (
            sum(r.value for r in glucose_records) / len(glucose_records)
            if glucose_records
            else None
        )
        ...
        weight_records = (
            self.db.query(WeightRecord)
            .filter(WeightRecord.user_id == self.user_id)
        ...
        avg_sleep = (
            sum(s.duration_hours for s in sleep_records) / len(sleep_records)
            if sleep_records
            else None
        )
        ...
        return {
            ...
            "weight": {
                ...
                "change_direction": (
                    ...
                    else "up" if weight_change and weight_change > 0 else "stable"
                ),
            },
            "sleep": {
            ...
            "medication": {"taken_count": med_logs_week},
        }
    ...
    def _calculate_health_score(self) -> int:
        ...
        goals = (
            ...
            .filter(HealthGoal.user_id == self.user_id, HealthGoal.status == "active")
            .all()
        )
    ...
    def _get_today_tasks(self) -> List[Dict[str, Any]]:
        ...
        for goal in goals:
            ...
            checked_in = (
                ...
                .filter(GoalCheckIn.goal_id == goal.id, GoalCheckIn.date == today)
                .first()
            )
    ...
    def _get_bp_trend(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend": (
                "improving"
                if records[-1].systolic < records[0].systolic
                else (
                    "stable"
                    if records[-1].systolic == records[0].systolic
                    else "worsening"
    ...
    def _get_weight_trend(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend": (
                "decreasing"
                if change < 0
                else "stable" if change == 0 else "increasing"
            ),
        }
    ...
    def _get_sleep_trend(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        return {
            "has_data": True,
            "avg_duration": round(
                sum(r.duration_hours for r in records) / len(records), 1
            ),
            "avg_quality": (
                round(sum(r.quality_score or 0 for r in records) / len(records))
                if records
                else None
            ),
        }
            ...
            result.append(
                {
                    ...
                    "days_remaining": (
                        (goal.deadline - date.today()).days if goal.deadline else None
                    ),
                }
            )
    ...
    def _get_recent_records(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        glucose = (
            self.db.query(GlucoseRecord)
            .filter(GlucoseRecord.user_id == self.user_id)
    ...
    def _get_medication_stats(self, for_date: date) -> Dict[str, Any]:
        ...
        return {
            ...
            "taken": taken,
            "completion_rate": (
                round(taken / total_doses * 100, 1) if total_doses > 0 else 100
            ),
        }
