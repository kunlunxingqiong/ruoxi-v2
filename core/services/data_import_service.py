...
class DataImportService:
    ...
    def import_from_json(self, json_data: Dict[str, Any],
                        merge_strategy: str = "skip_duplicates") -> DataImportResult:
        """
        从JSON导入数据
        ...
        try:
            ...
            if "health_records" in json_data:
                ...
                self._import_blood_pressure(health.get("blood_pressure", []), merge_strategy)
            ...
            if "medications" in json_data:
                self._import_medications(json_data["medications"].get("medications", []), merge_strategy)
    ...
    def import_from_csv(self, csv_data: str, record_type: str,
                       merge_strategy: str = "skip_duplicates") -> DataImportResult:
        """从CSV导入数据"""
        self.result = DataImportResult()
    ...
    def restore_from_backup(self, zip_bytes: bytes) -> DataImportResult:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, 'r') as zip_file:
                ...
                logger.info(f"恢复备份: 版本 {metadata.get('version')}, "
                ...
                if 'data/health_data.json' in zip_file.namelist():
                    ...
                    for filename in zip_file.namelist():
                        if filename.startswith('csv/') and filename.endswith('.csv'):
                            record_type = filename.replace('csv/', '').replace('.csv', '')
                            csv_content = zip_file.read(filename).decode('utf-8')
                            self.import_from_csv(csv_content, record_type, "skip_duplicates")
        ...
        for record_data in records:
            try:
                ...
                existing = self.db.query(GlucoseRecord).filter(
                    ...
                    GlucoseRecord.meal_type == record_data.get("meal_type", "fasting")
                ).first()
            ...
            try:
                ...
                if existing:
                    if merge_strategy == "skip_duplicates":
                        ...
                        existing.body_fat_percentage = record_data.get("body_fat_percentage")
    ...
    def _import_sleep(self, records: List[Dict], merge_strategy: str):
        ...
        for record_data in records:
            try:
                ...
                bedtime = datetime.fromisoformat(record_data["bedtime"]) if record_data.get("bedtime") else None
                wake_time = datetime.fromisoformat(record_data["wake_time"]) if record_data.get("wake_time") else None
            ...
            try:
                ...
                if not existing:
                    medication = Medication(
                        ...
                        end_date=date.fromisoformat(med_data["end_date"]) if med_data.get("end_date") else None,
    ...
    def _import_medication_log(self, medication_id: int, log_data: Dict, merge_strategy: str):
        """导入用药日志"""
        try:
    ...
    def _import_goals(self, goals: List[Dict], merge_strategy: str):
        ...
        for goal_data in goals:
            try:
                existing = self.db.query(HealthGoal).filter(
                ...
                goal = HealthGoal(
                    ...
                    start_date=date.fromisoformat(goal_data["start_date"]) if goal_data.get("start_date") else date.today(),
                    deadline=date.fromisoformat(goal_data["deadline"]) if goal_data.get("deadline") else None,
    ...
    def _import_check_in(self, goal_id: int, check_in_data: Dict, merge_strategy: str):
        ...
        try:
            ...
            existing = self.db.query(GoalCheckIn).filter(
                GoalCheckIn.goal_id == goal_id,
                GoalCheckIn.date == check_date
            ).first()
        ...
        for row in reader:
            try:
                ...
                record = WeightRecord(
                    ...
                    body_fat_percentage=float(row["体脂率(%)"]) if row.get("体脂率(%)") else None,
                    measured_at=measured_at,
                    note=row.get("备注") or None
                )
                self.db.add(record)
    ...
    def _import_sleep_csv(self, csv_content: str, merge_strategy: str):
        ...
        for row in reader:
            try:
                ...
                bedtime = datetime.strptime(row["入睡时间"], "%H:%M") if row.get("入睡时间") else None
                wake_time = datetime.strptime(row["起床时间"], "%H:%M") if row.get("起床时间") else None
                ...
                record = SleepRecord(
                    ...
                    bedtime=bedtime,
                    wake_time=wake_time,
                    deep_sleep_percentage=float(row["深睡(%)"]) if row.get("深睡(%)") else None,
                    light_sleep_percentage=float(row["浅睡(%)"]) if row.get("浅睡(%)") else None,
                    rem_sleep_percentage=float(row["REM(%)"]) if row.get("REM(%)") else None
                )
                self.db.add(record)
                self.result.add_success("sleep", record.id)
...
# 便捷函数
def import_user_data_json(db: Session, user_id: int, json_data: Dict[str, Any],
                          merge_strategy: str = "skip_duplicates") -> Dict[str, Any]:
    """从JSON导入数据便捷函数"""
