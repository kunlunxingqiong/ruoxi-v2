...
class AppleHealthImporter:
                ...
                try:
                    ...
                    records.append(
                        HealthDataRecord(
                            ...
                            value=converted_value,
                            unit=self._get_standard_unit(
                                self.RECORD_TYPE_MAP[stat_type]
                            ),
                            start_date=start_date,
                            end_date=end_date,
                            source_name=elem.get("sourceName", ""),
    ...
    def import_to_database(
        self, records: List[HealthDataRecord], skip_duplicates: bool = True
    ) -> Dict[str, Any]:
        """
        将解析的数据导入数据库
        ...
        # 导入各类数据
        import_counts = {
            "blood_pressure": self._import_blood_pressure(
                grouped.get("bp_pairs", []), skip_duplicates
            ),
            "heart_rate": self._import_heart_rate(
                grouped.get("heart_rate", []), skip_duplicates
            ),
            "weight": self._import_weight(grouped.get("weight", []), skip_duplicates),
            "sleep": self._import_sleep(grouped.get("sleep", []), skip_duplicates),
            "glucose": self._import_glucose(
                grouped.get("glucose", []), skip_duplicates
            ),
        }
    ...
    def _import_blood_pressure(
        self, bp_pairs: List[Dict], skip_duplicates: bool
    ) -> int:
        """导入血压记录"""
        count = 0
    ...
    def _import_heart_rate(
        self, records: List[HealthDataRecord], skip_duplicates: bool
    ) -> int:
        """导入心率记录"""
        count = 0
        ...
        for record in records:
            ...
            activity = (
                record.metadata.get("activity", "unknown")
                if record.metadata
                else "unknown"
            )
    ...
    def _import_sleep(
        ...
        for night_key, sleep_data in sleep_by_night.items():
            ...
            duration_minutes = int(
                (sleep_data["end"] - sleep_data["start"]).total_seconds() / 60
            )
...
def import_apple_health_data(
    file_path: str, user_id: int, db: Session, skip_duplicates: bool = True
) -> Dict[str, Any]:
    """
