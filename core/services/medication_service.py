...
from models.database import Medication, MedicationLog, Notification
...
class MedicationService:
    ...
    def get_due_medications(
        self, user_id: int, check_time: datetime
    ) -> List[Dict[str, Any]]:
        """
        获取在当前时间应该提醒的用药
    ...
    def get_medication_adherence(
        self, user_id: int, medication_id: Optional[int] = None, days: int = 30
    ) -> Dict[str, Any]:
        """
        计算用药依从性
        ...
        base_query = (
            self.db.query(MedicationLog)
            .join(Medication)
            .filter(Medication.user_id == user_id, MedicationLog.taken_at >= start_date)
        )
        ...
        adherence_rate = (
            (taken_count / expected_count * 100) if expected_count > 0 else 0
        )
    ...
    def get_medication_schedule(
        ...
        medications = (
            ...
            .filter(
                ...
                or_(Medication.end_date == None, Medication.end_date >= target_date),
            )
            .all()
        )
    ...
    def detect_missed_doses(self, user_id: int, days: int = 7) -> List[Dict[str, Any]]:
        """检测漏服记录"""
        start_date = date.today() - timedelta(days=days)
                    ...
                    missed_doses.append(
                        {
                            ...
                            "reminder_time": med.reminder_time,
                            "expected_time": (
                                f"{check_date} {med.reminder_time}"
    ...
    def get_medication_summary(self, user_id: int) -> Dict[str, Any]:
        """获取用药摘要"""
        # 活跃用药
        active_count = (
            self.db.query(Medication)
        ...
        return {
            ...
            "today_schedule": {
                ...
                "completion_rate": (
                    round(today_taken / len(today_schedule) * 100, 1)
                    if today_schedule
                    else 0
                ),
            },
    ...
    def generate_medication_report(
        ...
        # 各用药详细统计
        medications = (
            self.db.query(Medication).filter(Medication.user_id == user_id).all()
        )
