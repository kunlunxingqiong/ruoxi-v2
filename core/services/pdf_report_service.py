...
class PDFReportService:
    ...
    def _generate_health_overview(self, period: ReportPeriod) -> Dict[str, Any]:
        ...
        weight_count = self.db.query(WeightRecord).filter(
        ...
        sleep_count = self.db.query(SleepRecord).filter(
        ...
        hr_count = self.db.query(HeartRateRecord).filter(
        ...
        return {
            "data_summary": {
                ...
                "total_data_points": bp_count + glucose_count + weight_count + sleep_count + hr_count
            },
    ...
    def _calculate_hr_score(self, period: ReportPeriod) -> Optional[float]:
        """计算心率健康评分"""
        records = self.db.query(HeartRateRecord).filter(
            HeartRateRecord.user_id == self.user_id,
            HeartRateRecord.measured_at >= period.start_date,
            HeartRateRecord.measured_at < period.end_date + timedelta(days=1)
        ).all()
    ...
    def _generate_medication_section(self, period: ReportPeriod) -> Dict[str, Any]:
        ...
        for med in active_medications:
            logs = self.db.query(MedicationLog).filter(
            ...
            med_reports.append({
                ...
                "adherence_rate": round(adherence_rate, 1) if adherence_rate else None,
                "status": "excellent" if adherence_rate and adherence_rate >= 95 else
                         "good" if adherence_rate and adherence_rate >= 80 else
                         "fair" if adherence_rate and adherence_rate >= 50 else
                         "poor"
            })
    ...
    def _calculate_overall_adherence(self, med_reports: List[dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "excellent" if avg_rate >= 95 else
                      "good" if avg_rate >= 80 else
                      "fair" if avg_rate >= 50 else
                      "needs_improvement"
        }
