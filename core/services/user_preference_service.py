...
from models.database import NotificationSetting, User, UserPreference
...
class UserPreferenceService:
    ...
    def get_preference(self, key: str, default: Any = None) -> Any:
        ...
        pref = (
            self.db.query(UserPreference)
            .filter(UserPreference.user_id == self.user_id, UserPreference.key == key)
            .first()
        )
    ...
    def set_preference(self, key: str, value: Any) -> bool:
        ...
        try:
            pref = (
                self.db.query(UserPreference)
                .filter(
                    UserPreference.user_id == self.user_id, UserPreference.key == key
                )
                .first()
            )
            ...
            if pref:
                ...
                # 新建
                pref = UserPreference(
                    user_id=self.user_id, key=key, value=value_str, type=value_type
                )
                self.db.add(pref)
    ...
    def get_privacy_settings(self) -> Dict[str, bool]:
        """获取隐私设置"""
        return {
            "share_anonymous_data": self.get_preference(
                "share_anonymous_data", PreferenceDefaults.SHARE_ANONYMOUS_DATA
            ),
            "allow_ai_analysis": self.get_preference(
...
class NotificationPreferenceService:
    ...
    def get_notification_settings(self) -> Dict[str, Any]:
        ...
        settings = (
            ...
            .filter(NotificationSetting.user_id == self.user_id)
            .first()
        )
