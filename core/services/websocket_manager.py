...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: int, client_info: dict = None):
        """
        建立WebSocket连接
    ...
    async def check_health_alert(
        ...
        if record_type == 'blood_pressure':
            ...
            # 血压危机
            if systolic >= thresholds['crisis_systolic'] or diastolic >= thresholds['crisis_diastolic']:
                alert = {
            ...
            # 高血压
            elif systolic >= thresholds['high_systolic'] or diastolic >= thresholds['high_diastolic']:
                alert = {
                    'type': NotificationType.HEALTH_ALERT,
    ...
    async def send_goal_achievement(self, user_id: int, goal_type: str, achievement: dict):
        """
        发送目标达成祝贺
    ...
    async def handle_client_message(self, user_id: int, websocket: WebSocket, message: dict):
        """
        处理客户端消息
