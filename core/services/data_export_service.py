...
class DataExportService:
    ...
    def _export_json(self, start_date: Optional[datetime] = None,
                     end_date: Optional[datetime] = None) -> Dict[str, Any]:
        """导出为JSON格式"""
        data = {
            "export_metadata": {
    ...
    def _export_csv_zip(self, start_date: Optional[datetime] = None,
                        end_date: Optional[datetime] = None) -> Dict[str, Any]:
        """导出为多个CSV文件的ZIP包"""
    ...
    def _export_full_backup(self, start_date: Optional[datetime] = None,
                           end_date: Optional[datetime] = None) -> Dict[str, Any]:
        """导出完整备份（JSON + CSV + 元数据）"""
        zip_buffer = io.BytesIO()
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            zip_file.writestr('data/health_data.json', 
    ...
    def _export_bp_csv(self, start_date: Optional[datetime], 
        ...
        writer.writerow(['记录时间', '收缩压(mmHg)', '舒张压(mmHg)', '心率', '状态', '备注'])
    ...
    def _export_glucose_csv(self, start_date: Optional[datetime],
                            end_date: Optional[datetime]) -> Optional[str]:
        """导出血糖记录为CSV"""
    ...
    def _export_weight_csv(self, start_date: Optional[datetime],
                           end_date: Optional[datetime]) -> Optional[str]:
        """导出体重记录为CSV"""
        query = self.db.query(WeightRecord).filter(
    ...
    def _export_hr_csv(self, start_date: Optional[datetime],
                       end_date: Optional[datetime]) -> Optional[str]:
        """导出心率记录为CSV"""
    ...
    def _export_sleep_csv(self, start_date: Optional[datetime],
                          end_date: Optional[datetime]) -> Optional[str]:
        """导出睡眠记录为CSV"""
        query = self.db.query(SleepRecord).filter(
        ...
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(['日期', '睡眠时长(小时)', '睡眠评分', '入睡时间', '起床时间', '深睡(%)', '浅睡(%)', 'REM(%)'])
    ...
    def _export_medication_csv(self, start_date: Optional[datetime],
                               end_date: Optional[datetime]) -> Optional[str]:
        """导出用药记录为CSV"""
        # 用药日志
        query = self.db.query(MedicationLog).join(Medication).filter(
    ...
    def _get_health_records(self, start_date: Optional[datetime],
                           end_date: Optional[datetime]) -> Dict[str, List]:
        """获取健康记录"""
        records = {
            "blood_pressure": [],
            "glucose": [],
        ...
        if start_date:
            glucose_query = glucose_query.filter(GlucoseRecord.measured_at >= start_date)
        if end_date:
    ...
    def _get_medications(self, start_date: Optional[datetime],
                        end_date: Optional[datetime]) -> Dict[str, List]:
        """获取用药数据"""
        medications = []
    ...
    def _get_goals(self, start_date: Optional[datetime],
                   end_date: Optional[datetime]) -> Dict[str, List]:
        """获取健康目标"""
        goals = []
        
        query = self.db.query(HealthGoal).filter(
    ...
    def _get_reports(self, start_date: Optional[datetime],
                    end_date: Optional[datetime]) -> Dict[str, List]:
        """获取健康报告"""
        reports = []
        
        query = self.db.query(HealthReport).filter(
            HealthReport.user_id == self.user_id
        )
    ...
    def _get_notifications(self, start_date: Optional[datetime],
                          end_date: Optional[datetime]) -> Dict[str, List]:
        """获取通知历史"""
        notifications = []
...
# 便捷函数
def export_user_data(db: Session, user_id: int, format: str = "json",
                     start_date: Optional[datetime] = None,
                     end_date: Optional[datetime] = None) -> Dict[str, Any]:
    """导出用户数据便捷函数"""
