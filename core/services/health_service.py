...
class HealthService:
    ...
    def get_bp_statistics(
        ...
        return {
            ...
            "standard_deviation": {
                "systolic": round(statistics.stdev(systolic_values), 2) if len(systolic_values) > 1 else 0,
                "diastolic": round(statistics.stdev(diastolic_values), 2) if len(diastolic_values) > 1 else 0
            },
            "category_distribution": category_counts,
            "abnormal_count": sum(
                category_counts.get(cat, 0) 
                for cat in ["stage1", "stage2", "crisis"]
            )
        }
    ...
    def get_morning_bp_surge(
        ...
        morning_records = [
            r for r in records 
            if time(6, 0) <= r.measured_at.time() <= time(10, 0)
        ]
        other_records = [
            r for r in records 
    ...
    def calculate_weight_progress(
        ...
        return {
            ...
            "records_count": len(records),
            "trend": "decreasing" if total_change < -0.5 else "increasing" if total_change > 0.5 else "stable"
        }
    ...
    def _generate_sleep_recommendations(
        ...
        if deep_sleep_records:
            avg_deep_ratio = statistics.mean([
                r.deep_sleep_minutes / r.duration_minutes 
                for r in deep_sleep_records
            ])
    ...
    def get_heart_rate_variability(
        ...
        resting_records = self.db.query(HeartRateRecord).filter(
    ...
    def get_health_score(
        ...
        if glucose_trend["records_count"] > 0:
            ...
            scores["glucose"] = round(statistics.mean(control_rates)) if control_rates else None
        else:
            scores["glucose"] = None
        ...
        if hrv:
            hrv_scores = {"excellent": 100, "good": 85, "average": 70, "below_average": 50}
        ...
        final_score = round(total_score * (100 / total_weight)) if total_weight > 0 else None
