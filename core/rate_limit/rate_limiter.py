...
class RateLimiter:
    ...
    def configure(
        ...
        if strategy == RateLimitStrategy.TOKEN_BUCKET:
            self._token_buckets[name] = TokenBucket(
                rate=requests / window_seconds, capacity=burst or requests
            )

    async def check(self, config_name: str, key: str) -> Tuple[bool, Dict]:
        """
        检查限流
        ...
        if config.strategy == RateLimitStrategy.SLIDING_WINDOW:
            allowed, remaining, retry_after = await self._sliding_window.is_allowed(
                f"{config_name}:{key}", config.requests
            )
    ...
    def limit(self, config_name: str, key_func: Optional[Callable] = None):
        """
        限流装饰器
...
DEFAULT_LIMITS = {
    ...
    "chat": RateLimitConfig(
        requests=30, window_seconds=60, strategy=RateLimitStrategy.TOKEN_BUCKET, burst=5
    ),
