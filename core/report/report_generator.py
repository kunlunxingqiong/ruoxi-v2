...
class ReportGenerator:
    ...
    async def _parse_health_data(
        ...
        bp_avg_dia = None
        if bp_readings:
            bp_avg_sys = sum(r.get("systolic", 0) for r in bp_readings) / len(bp_readings)
            bp_avg_dia = sum(r.get("diastolic", 0) for r in bp_readings) / len(bp_readings)
        
        # 解析血糖
        glucose_readings = health_data.get("blood_glucose", [])
        glucose_avg = None
        if glucose_readings:
            glucose_avg = sum(r.get("value", 0) for r in glucose_readings) / len(glucose_readings)
        ...
        sleep_eff = None
        if sleep_readings:
            sleep_avg = sum(r.get("duration_hours", 0) for r in sleep_readings) / len(sleep_readings)
            sleep_eff = sum(r.get("efficiency", 0) for r in sleep_readings) / len(sleep_readings)
    ...
    async def _generate_ai_insights(self, data: HealthReportData):
        ...
        if data.emotion_summary:
            negative = data.emotion_summary.get("sad", 0) + data.emotion_summary.get("anxious", 0)
            if negative > 3:
