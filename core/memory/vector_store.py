...
class EmbeddingService:
    ...
    def _load_model(self):
        ...
        # 使用缓存目录
        cache_dir = Path(config.get("system.data_dir", "data")) / "models" / "embeddings"
    ...
    def encode(self, texts: Union[str, List[str]]) -> List[List[float]]:
        ...
        if self.is_available and self.model:
            try:
                ...
                return embeddings if isinstance(embeddings[0], list) else [embeddings.tolist()]
            except Exception as e:
                ...
                for i, memory_id in enumerate(results['ids'][0]):
                    ...
                    memory = MemoryItem(
                        ...
                        metadata={k: v for k, v in metadata.items() 
                                if k not in ['memory_type', 'user_id', 'session_id', 'importance', 'created_at']},
            ...
            if results['ids']:
                ...
                for i, memory_id in enumerate(results['ids']):
                    created_at = datetime.fromisoformat(
                        results['metadatas'][i].get('created_at', datetime.utcnow().isoformat())
                    )
...
if __name__ == "__main__":
    ...
    if CHROMADB_AVAILABLE:
        ...
        for r in results:
            print(f"    - [{r.memory_type}] {r.content[:40]}... (重要度: {r.importance})")
