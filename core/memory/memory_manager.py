...
class MemoryManager:
    ...
    def save_conversation_to_memory(
        ...
        for msg in user_messages[-3:]:  # 最近3条用户消息
            ...
            # 生成唯一ID
            memory_id = f"mem_{user_id}_{hashlib.md5(content.encode()).hexdigest()[:12]}"
...
if __name__ == "__main__":
    ...
    # 添加对话
    memory_manager.add_to_context(session_id, user_id, "user", "你好若曦，我喜欢喝抹茶拿铁")
    memory_manager.add_to_context(session_id, user_id, "assistant", "🌸 曦曦记住啦~ 抹茶拿铁不加糖？")
