...
class GlobalExceptionHandler:
    ...
    def create_error_response(
        ...
        # 添加友好的用户消息
        error_dict["friendly_message"] = GlobalExceptionHandler._get_friendly_message(error_dict)
    ...
    def _get_friendly_message(error_dict: Dict) -> str:
        ...
        return friendly_messages.get(
            code,
            error_dict.get("message", "出了点小问题，请稍后重试~ 🌸")
        )
            ...
            async def wrapper(*args, **kwargs):
                ...
                if func_name in failures:
                    ...
                    if failure_info["count"] >= threshold:
                        if __import__('time').time() - failure_info["last_time"] < timeout:
