...
class HealthChartGenerator:
    ...
    def generate_glucose_chart_data(
        self,
        readings: List[Dict],
        days: int = 7
    ) -> Dict:
    ...
    def generate_emotion_chart_data(
        ...
        data = [
            ...
            for k, v in emotions.items() if v > 0
        ]
    ...
    def generate_comprehensive_dashboard(
        ...
        return {
            ...
            "summary": {
                "bp_avg": self._calc_avg(health_data.get("blood_pressure", []), "systolic"),
                "glucose_avg": self._calc_avg(health_data.get("blood_glucose", []), "value"),
                "sleep_avg": self._calc_avg(health_data.get("sleep", []), "duration_hours"),
                "emotion_dominant": max(
                    emotion_data.get("emotion_summary", {}).items(),
                    key=lambda x: x[1]
                )[0] if emotion_data.get("emotion_summary") else "neutral"
            }
        }
