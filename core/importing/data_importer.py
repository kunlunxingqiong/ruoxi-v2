...
class DataImporter:
    ...
    async def import_from_file(
        ...
        try:
            ...
            report.import_duration_ms = int((end_time - start_time).total_seconds() * 1000)
    ...
    async def _import_records(
        ...
        for i, record in enumerate(records):
            try:
                # 自动检测数据类型
                detected_type = self._detect_data_type(record) if data_type == "auto_detect" else data_type
    ...
    def _standardize_record(
        ...
        # 提取时间戳
        timestamp = record.get('timestamp') or record.get('date') or record.get('start_date')
        if timestamp:
        ...
        if data_type == 'blood_pressure':
            ...
            standardized["value"] = float(record.get('glucose') or record.get('blood_glucose', 0))
        elif data_type == 'heart_rate':
            standardized["bpm"] = float(record.get('heart_rate') or record.get('bpm', 0))
        elif data_type == 'sleep':
            standardized["duration_hours"] = float(record.get('sleep_duration') or record.get('duration_hours', 0))
