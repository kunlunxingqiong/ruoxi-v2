...
class VoiceManager:
    ...
    async def _azure_tts(self, text: str, profile: VoiceProfile) -> Optional[bytes]:
        """Azure TTS"""
        # 需要API密钥
        return None

    async def _local_tts(self, text: str, profile: VoiceProfile) -> Optional[bytes]:
        """本地TTS (如 pyttsx3)"""
        try:
            import pyttsx3
    ...
    async def stream_text_to_speech(
        self, text_stream: asyncio.Queue, audio_queue: asyncio.Queue
    ):
        """流式语音合成"""
        buffer = ""
    ...
    def add_custom_profile(self, name: str, profile: VoiceProfile):
        """添加自定义声音"""
