...
class DataExporter:
    ...
    async def export_health_records(
        ...
        if date_range:
            ...
            records = [
                ...
                for r in records
                if start
                <= datetime.fromisoformat(r.get("timestamp", "2000-01-01"))
                <= end
            ]
    ...
    async def export_chat_history(
        ...
        if date_range:
            ...
            messages = [
                ...
                for m in messages
                if start
                <= datetime.fromisoformat(m.get("timestamp", "2000-01-01"))
                <= end
            ]
    ...
    async def create_full_backup(
        self, user_id: str, data: Dict[str, List[Dict]]
    ) -> str:
        """创建完整备份"""
    ...
    async def _export_json(self, user_id: str, data_type: str, data: List[Dict]) -> str:
        """导出为JSON"""
    ...
    async def _export_csv(self, user_id: str, data_type: str, data: List[Dict]) -> str:
        """导出为CSV"""
        timestamp = datetime.now().strftime("%Y%m%d")
    ...
    async def _export_health_md(self, user_id: str, records: List[Dict]) -> str:
        """导出健康记录为Markdown"""
        timestamp = datetime.now().strftime("%Y%m%d")
...
"""
    ...
    async def _export_chat_md(self, user_id: str, messages: List[Dict]) -> str:
        """导出聊天记录为Markdown"""
        timestamp = datetime.now().strftime("%Y%m%d")
