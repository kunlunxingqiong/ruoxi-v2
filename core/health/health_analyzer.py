...
class HealthAnalyzer:
    ...
    def analyze_blood_pressure(
        self, records: List[HealthMetric]
        ...
        if sys_val and dia_val:
            if (
                ranges["normal"]["systolic"][0]
                <= sys_val
                <= ranges["normal"]["systolic"][1]
                and ranges["normal"]["diastolic"][0]
                <= dia_val
                <= ranges["normal"]["diastolic"][1]
            ):
                status = "正常"
                risk = "低"
                summary = f"血压在正常范围 ({sys_val}/{dia_val} mmHg)，继续保持！"
            elif (
                ranges["elevated"]["systolic"][0]
                <= sys_val
                <= ranges["elevated"]["systolic"][1]
                or ranges["elevated"]["diastolic"][0]
                <= dia_val
                <= ranges["elevated"]["diastolic"][1]
            ):
                status = "正常高值"
                risk = "中"
    ...
    def analyze_blood_glucose(
        self, records: List[HealthMetric], test_type: str = "fasting"
    ) -> HealthAnalysisResult:
        """分析血糖数据"""
        if not records:
    ...
    def analyze_sleep(self, records: List[HealthMetric]) -> HealthAnalysisResult:
        """分析睡眠数据"""
        if not records:
        ...
        trend = self._analyze_trend(
            durations[-7:] if len(durations) >= 7 else durations
        )

        suggestions = []
        if avg_duration < 6:
            suggestions.extend(
                ["尽量在23点前入睡", "睡前1小时不看手机", "创造舒适的睡眠环境"]
            )
        else:
    ...
    async def generate_health_report(
        self, user_id: str, health_records: Dict[str, List[HealthMetric]]
    ) -> str:
        """
        生成综合健康报告 (AI驱动)
        ...
        if "blood_pressure" in health_records:
            analyses.append(
                self.analyze_blood_pressure(health_records["blood_pressure"])
            )
        ...
        prompt_data = {
            ...
            "high_risk": [a.metric_type for a in analyses if a.risk_level == "高"],
        }
...
"""
    ...
    async def answer_health_question(
        self, question: str, user_health_data: Dict
    ) -> str:
        """
        回答健康问题 (结合用户数据)
...
"""
    ...
    def _generate_bp_suggestions(self, status: str, trend: str) -> List[str]:
        ...
        if status in ["偏高", "正常高值"]:
            suggestions.extend(
                ["减少盐摄入，每天不超过6克", "多吃蔬菜水果", "每周运动150分钟以上"]
            )
...
if __name__ == "__main__":
    ...
    bp_records = [
        HealthMetric(
            "blood_pressure", 0, "mmHg", datetime.now(), systolic=125, diastolic=82
        ),
        HealthMetric(
            "blood_pressure", 0, "mmHg", datetime.now(), systolic=128, diastolic=85
        ),
        HealthMetric(
            "blood_pressure", 0, "mmHg", datetime.now(), systolic=118, diastolic=78
        ),
    ]
