...
class MedicationCreate(BaseModel):
    ...
    reminder_time: Optional[str] = Field(
        None, max_length=50, description="提醒时间，如「08:00」"
    )
...
async def get_medication(
    ...
    medication = (
        db.query(Medication)
        .filter(
            Medication.id == medication_id, Medication.user_id == current_user.user_id
        )
        .first()
    )
...
async def get_today_schedule(
    ...
    schedule = service.get_medication_schedule(
        user_id=current_user.user_id, target_date=date.today()
    )
    ...
    overdue = sum(
        ...
        > datetime.strptime(s["reminder_time"], "%H:%M").time()
    )
    ...
    return {
        ...
        "summary": {
            ...
            "overdue": overdue,
            "completion_rate": (
                round(len(taken) / len(schedule) * 100, 1) if schedule else 0
            ),
        },
        "schedule": schedule,
    }
...
async def get_due_reminders(
    ...
    due = service.get_due_medications(
        user_id=current_user.user_id, check_time=datetime.now()
    )
...
async def get_adherence_summary(
    medication_id: Optional[int] = Query(
        None, description="指定用药ID，不传则统计全部"
    ),
    ...
    adherence = service.get_medication_adherence(
        user_id=current_user.user_id, medication_id=medication_id, days=days
    )

    return {"success": True, "medication_id": medication_id, "adherence": adherence}
...
async def get_missed_doses(
    ...
    """
    service = MedicationService(db)
    missed = service.detect_missed_doses(user_id=current_user.user_id, days=days)
...
async def generate_medication_report(
    ...
    """
    service = MedicationService(db)
    report = service.generate_medication_report(user_id=current_user.user_id, days=days)
