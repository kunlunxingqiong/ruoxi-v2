...
async def get_single_metric_prediction(
    metric: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    获取单项指标预测
    ...
    return {"success": True, "metric": metric, "prediction": result}
...
async def get_single_disease_risk(
    disease: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    获取单项疾病风险详情
    ...
    for risk in risks.get("risks", []):
        if risk.get("disease_name") == disease_name:
            return {"success": True, "disease": disease, "risk_assessment": risk}

    return {"success": False, "message": f"无法评估{disease_name}风险，数据不足"}
            ...
            insights.append(
                {
                    ...
                    "severity": (
                        "warning" if risk.get("risk_level") == "high" else "critical"
                    ),
