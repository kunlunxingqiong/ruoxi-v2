...
from core.health_timeline.timeline_engine import ChartType, TimeRange, timeline_engine

router = APIRouter(prefix="/timeline", tags=["健康时间线"])


@router.get("/views")
async def get_available_views(current_user=Depends(get_current_user)):
    """
    获取可用的时间线视图
...
async def get_timeline_events(
    ...
    importance: Optional[str] = Query(
        None, description="重要性过滤: low, normal, high, critical"
    ),
    current_user=Depends(get_current_user),
):
    """
    ...
    events = await timeline_engine._fetch_events(
        user_id=str(current_user.user_id), start_date=start, end_date=end
    )
    ...
    return {
        ...
        "total": len(events),
        "filters": {"event_type": event_type, "importance": importance},
    }
