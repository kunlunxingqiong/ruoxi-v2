...
class HeartRateCreate(BaseModel):
    ...
    activity: Optional[str] = Field(
        "resting", description="活动状态: resting/walking/exercising"
    )
    measured_at: datetime = Field(default_factory=datetime.now)
    note: Optional[str] = Field(None, max_length=500)
...
async def list_bp_records(
    ...
    order_column = getattr(
        BloodPressureRecord, params.order_by, BloodPressureRecord.measured_at
    )
...
async def get_latest_bp_stats(
    ...
    if today_records:
        today_avg = {
            "systolic": round(
                sum(r.systolic for r in today_records) / len(today_records), 1
            ),
            "diastolic": round(
                sum(r.diastolic for r in today_records) / len(today_records), 1
            ),
    ...
    if week_records:
        week_avg = {
            "systolic": round(
                sum(r.systolic for r in week_records) / len(week_records), 1
            ),
            "diastolic": round(
                sum(r.diastolic for r in week_records) / len(week_records), 1
            ),
...
async def get_glucose_record(
    ...
    record = (
        ...
        .filter(
            GlucoseRecord.id == record_id, GlucoseRecord.user_id == current_user.user_id
        )
        .first()
    )
...
async def list_weight_records(
    ...
    """获取体重记录列表"""
    query = db.query(WeightRecord).filter(WeightRecord.user_id == current_user.user_id)
...
async def get_weight_record(
    ...
    record = (
        ...
        .filter(
            WeightRecord.id == record_id, WeightRecord.user_id == current_user.user_id
        )
        .first()
    )
...
async def list_sleep_records(
    ...
    """获取睡眠记录列表"""
    query = db.query(SleepRecord).filter(SleepRecord.user_id == current_user.user_id)
...
async def get_sleep_record(
    ...
    record = (
        ...
        .filter(
            SleepRecord.id == record_id, SleepRecord.user_id == current_user.user_id
        )
        .first()
    )
...
async def get_sleep_summary(
    ...
    avg_quality = (
        sum(r.sleep_quality for r in records if r.sleep_quality)
        / len([r for r in records if r.sleep_quality])
        if any(r.sleep_quality for r in records)
        else None
    )
    ...
    return {
        ...
        "sleep_debt_hours": max(
            0, round(7.5 * days - sum(r.duration_minutes for r in records) / 60, 1)
        ),
    }
...
async def list_hr_records(
    activity: Optional[str] = Query(
        None, description="按活动状态筛选: resting/walking/exercising"
    ),
    params: RecordQueryParams = Depends(),
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
...
async def get_health_overview(
    ...
    latest_glucose = (
        ...
        .order_by(desc(GlucoseRecord.measured_at))
        .first()
    )
    ...
    latest_weight = (
        ...
        .order_by(desc(WeightRecord.measured_at))
        .first()
    )
    ...
    latest_sleep = (
        ...
        .order_by(desc(SleepRecord.bed_time))
        .first()
    )
    ...
    latest_hr = (
        ...
        .order_by(desc(HeartRateRecord.measured_at))
        .first()
    )
    ...
    glucose_count_7d = (
        db.query(GlucoseRecord)
        .filter(
            GlucoseRecord.user_id == user_id, GlucoseRecord.measured_at >= last_7_days
        )
        .count()
    )
