...
async def broadcast_message(message: str, exclude_user: Optional[int] = None):
    """
    广播消息（管理员用）
...
async def update_health_threshold(metric: str, thresholds: dict):
    """
    更新健康监控阈值（高级用户/管理员用）
    ...
    if metric not in manager.health_thresholds:
        return {"success": False, "error": f"不支持的指标类型: {metric}"}
...
async def get_client_websocket_config():
    ...
    return {
        "success": True,
        "config": {
            "endpoints": {"health": "/ws/health", "notifications": "/ws/notifications"},
            "reconnect": {
                "enabled": True,
                "max_attempts": 5,
            ...
            "heartbeat": {"enabled": True, "interval_ms": 30000, "timeout_ms": 10000},
            "message_types": {
...
async def monitor_health_record(user_id: int, record_type: str, values: dict):
    """
    监控健康记录并触发警报
