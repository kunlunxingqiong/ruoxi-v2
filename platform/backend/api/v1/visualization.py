...
from core.scheduler.task_scheduler import task_scheduler, ruoxi_reminder_callback, TaskType
...
async def get_bp_chart(
    ...
    # TODO: 从数据库获取真实数据
    mock_data = [
        {"systolic": 118, "diastolic": 78, "timestamp": (datetime.utcnow() - datetime.timedelta(days=i)).isoformat()}
...
async def get_glucose_chart(
    ...
    """获取血糖趋势图表数据"""
    mock_data = [
        {"value": 5.2 + (i % 3) * 0.3, "timestamp": (datetime.utcnow() - datetime.timedelta(days=i)).isoformat()}
        for i in range(days, 0, -1)
    ]
