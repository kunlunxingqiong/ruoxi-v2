...
from core.ai.ai_model_manager import (
    ai_model_manager,
    ChatMessage,
    ModelCapability
)
...
async def translate_text(
    ...
    try:
        messages = [
            ChatMessage(role="system", content=f"You are a translator. Translate the following text to {target_language}. Only output the translation, no explanations."),
...
async def summarize_text(
    ...
    try:
        messages = [
            ChatMessage(role="system", content=f"Summarize the following text in {max_length} characters or less. Be concise and capture the main points."),
            ChatMessage(role="user", content=text)
        ]
...
回答要简洁专业，避免过度医疗建议。"""
        ...
        messages = [
            ChatMessage(role="system", content="你是一位专业的AI健康助手，提供准确的健康数据分析和建议。注意：这只是参考建议，不能替代医生诊断。"),
