...
from core.chat.chat_engine import ChatMode, chat_service
...
async def send_message(request: ChatRequest, current_user=Depends(get_current_user)):
    ...
    await connection_manager.send_to_user(
        str(current_user.user_id), {"type": "chat_message", "message": response}
    )
...
async def get_chat_history(
    limit: int = Query(50, ge=1, le=100), current_user=Depends(get_current_user)
):
    """获取聊天历史"""
    messages = await chat_service.get_history(
        user_id=str(current_user.user_id), limit=limit
    )

    return ChatHistoryResponse(success=True, messages=messages, total=len(messages))
...
async def emotion_checkin(
    mood: str, note: Optional[str] = None, current_user=Depends(get_current_user)
):
    """
    情绪打卡
...
async def get_chat_suggestions(current_user=Depends(get_current_user)):
    ...
    return {"success": True, "suggestions": suggestions[:6]}


@router.post("/voice")
async def voice_chat(audio_data: bytes, current_user=Depends(get_current_user)):
    """语音聊天（TTS/STT）"""
    # TODO: 集成语音识别和合成
    return {"success": True, "message": "语音功能开发中", "note": "请使用文字聊天"}
