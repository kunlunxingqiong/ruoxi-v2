...
class MemoryCreate(BaseModel):
    ...
    memory_type: str = Field(..., description="记忆类型: fact/event/preference/emotion/goal")
...
async def create_memory(memory: MemoryCreate):
    ...
    **请求示例:**
    ...
    memories_db[memory_id] = {
        ...
        "summary": memory.summary or memory.content[:50] + "..." if len(memory.content) > 50 else memory.content,
        "importance": memory.importance,
        "created_at": datetime.utcnow(),
        "updated_at": datetime.utcnow()
    }
    
    logger.info(f"💾 记忆创建 | {memory_id} | 类型: {memory.memory_type} | 用户: {memory.user_id}")
...
async def list_memories(
    ...
    # 按类型筛选
    if memory_type:
        user_memories = [mem for mem in user_memories if mem["memory_type"] == memory_type]
    
    # 按重要程度筛选
    user_memories = [mem for mem in user_memories if mem["importance"] >= min_importance]
...
async def search_memories(
    ...
    for mem in memories_db.values():
        ...
        if score > 0:
            matching_memories.append({"memory": mem, "score": score + mem["importance"] / 10})
...
async def get_memory_stats(user_id: str):
    ...
    return {
        ...
        "last_updated": max([m["updated_at"].isoformat() for m in user_memories]) if user_memories else None
    }
