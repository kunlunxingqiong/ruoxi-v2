...
from ...core_auth import UserAuth, auth_manager, get_current_user, optional_current_user
...
async def logout(user: UserAuth = Depends(get_current_user)):
    ...
    return {"success": True, "message": "登出成功", "user_id": user.user_id}
...
async def update_me(
    ...
    return UserProfile(
        ...
        created_at=(
            user_data.get("created_at").isoformat()
...
async def forgot_password(username: str):
    ...
    return {"success": True, "message": "密码重置邮件已发送 (开发中功能)"}
