...
from core.data_import_export.exporter import (
    data_exporter,
    ExportFormat,
    ExportTemplate
)
...
async def preview_import(
    ...
    data_type: str = Query(..., description="数据类型: blood_pressure, blood_glucose, weight, sleep, heart_rate, steps, medication"),
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"无效数据类型，可选: {[t.value for t in DataType]}")
...
async def get_supported_import_formats(
    ...
    return {
        "success": True,
        "formats": [
            {"id": "csv", "name": "CSV", "extensions": [".csv"], "description": "逗号分隔值文件"},
            {"id": "excel", "name": "Excel", "extensions": [".xlsx", ".xls"], "description": "Microsoft Excel文件"},
            {"id": "json", "name": "JSON", "extensions": [".json"], "description": "JSON格式数据"}
        ],
        "data_types": [
            {"id": "blood_pressure", "name": "血压", "description": "收缩压/舒张压/脉搏", "example": "120/80/72"},
            {"id": "blood_glucose", "name": "血糖", "description": "血糖数值及时间", "example": "5.6 mmol/L (空腹)"},
...
async def export_data(
    ...
    template: str = Query("raw_data", description="模板: raw_data, daily_summary, weekly_report, monthly_report, for_doctor"),
    ...
    if not result.success:
        raise HTTPException(status_code=500, detail=result.warnings[0] if result.warnings else "导出失败")
...
async def export_health_report(
    ...
    if not result.success:
        raise HTTPException(status_code=500, detail=result.warnings[0] if result.warnings else "报告生成失败")
...
async def get_supported_export_formats(
    ...
    return {
        "success": True,
        "formats": [
            {"id": "csv", "name": "CSV", "mime_type": "text/csv", "description": "纯文本格式，通用性强"},
            {"id": "excel", "name": "Excel", "mime_type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "description": "格式化Excel表格，带样式"},
            {"id": "json", "name": "JSON", "mime_type": "application/json", "description": "JSON格式，适合程序处理"},
            {"id": "pdf", "name": "PDF", "mime_type": "application/pdf", "description": "PDF报告（开发中）"}
        ],
        "templates": [
