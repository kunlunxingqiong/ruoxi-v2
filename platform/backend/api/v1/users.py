...
from models.database import (
    get_db, User as UserModel, Gender
)
...
async def update_my_profile(
    ...
    if "gender" in update_data and update_data["gender"]:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"无效的性别值: {update_data['gender']}")
...
async def upload_avatar(
    ...
    if file.content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="不支持的文件格式，请上传 jpg, png 或 gif")
...
async def get_health_summary(
    ...
    glucose_count = db.query(GlucoseRecord).filter(
