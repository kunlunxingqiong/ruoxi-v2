...
async def generate_report(
    ...
    if request.report_type == "daily":
        ...
        if not request.start_date or not request.end_date:
            raise HTTPException(
                status_code=400, detail="自定义报告需要提供开始和结束日期"
            )
    ...
    try:
        ...
        return {
            ...
            "preview": {
                "health_score": report_data.get("health_overview", {}).get(
                    "health_score"
                ),
                "data_summary": report_data.get("health_overview", {}).get(
                    "data_summary"
                ),
...
async def get_weekly_report(
    ...
    try:
        ...
        return {"success": True, "report_type": "weekly", "data": report_data}
    except Exception as e:
...
async def get_monthly_report(
    ...
    try:
        ...
        return {"success": True, "report_type": "monthly", "data": report_data}
    except Exception as e:
    ...
    try:
        if metric == "bp":
            records = (
                ...
                .group_by(func.date(BloodPressureRecord.measured_at))
                .all()
            )
