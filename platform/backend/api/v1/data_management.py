...
from core.services.data_import_service import DataImportService, import_user_data_json, restore_user_backup
...
async def import_json_data(
    data: Dict[str, Any],
    merge_strategy: str = Query("skip_duplicates", enum=["skip_duplicates", "overwrite", "append"]),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
...
async def import_csv_data(
    ...
    record_type: str = Query(..., enum=["blood_pressure", "glucose", "weight", "heart_rate", "sleep"]),
    merge_strategy: str = Query("skip_duplicates", enum=["skip_duplicates", "overwrite"]),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
...
async def get_data_statistics(
    ...
    stats = {
        ...
        "medication_logs": db.query(MedicationLog).join(Medication).filter(
            Medication.user_id == current_user.user_id
        ).count(),
        "goals": db.query(HealthGoal).filter(
            HealthGoal.user_id == current_user.user_id
        ).count()
    }
    
    # 计算数据时间跨度
    first_bp = db.query(BloodPressureRecord).filter(
        BloodPressureRecord.user_id == current_user.user_id
    ).order_by(BloodPressureRecord.measured_at).first()
    
    last_bp = db.query(BloodPressureRecord).filter(
        BloodPressureRecord.user_id == current_user.user_id
    ).order_by(BloodPressureRecord.measured_at.desc()).first()
    
    total_records = sum(stats["health_records"].values()) + stats["medication_logs"] + stats["goals"]
