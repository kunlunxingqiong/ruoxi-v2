...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str = "anonymous"):
        ...
        logger.info(f"🔌 WebSocket连接 | 用户: {user_id} | 当前连接: {len(self.active_connections)}")
    ...
    def disconnect(self, websocket: WebSocket, user_id: str = "anonymous"):
        ...
        logger.info(f"🔌 WebSocket断开 | 用户: {user_id} | 剩余连接: {len(self.active_connections)}")
...
class ChatWebSocket:
    ...
    async def handle(self, websocket: WebSocket, session_id: str, user_id: str = "anonymous"):
        """
        处理WebSocket连接
    ...
    async def _handle_chat_message(
        ...
        logger.info(f"💬 WS消息 | 会话: {session_id} | 用户: {user_id} | 长度: {len(content)}")
