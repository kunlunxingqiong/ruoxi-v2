...
from sqlalchemy import create_engine, Column, String, Integer, DateTime, Float, Boolean, Text, ForeignKey, Index, Enum
...
class BloodPressureRecord(Base):
    ...
    # 索引
    __table_args__ = (
        Index('idx_bp_user_measured', 'user_id', 'measured_at'),
    )
...
class HeartRateRecord(Base):
    ...
    activity = Column(String(50), nullable=True)  # 活动状态: resting, walking, exercising
...
class HealthGoal(Base):
    ...
    goal_type = Column(String(50), nullable=False)  # weight, bp, glucose, sleep, exercise, etc.
...
class Memory(Base):
    ...
    # 记忆内容
    content = Column(Text, nullable=False)
    memory_type = Column(String(50), default="general")  # general, health, preference, fact
...
class Notification(Base):
    ...
    notification_type = Column(String(50), nullable=False)  # medication, health_alert, system, chat
