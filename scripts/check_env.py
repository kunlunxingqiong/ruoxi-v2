...
def check_python_version():
    ...
    if version.major >= 3 and version.minor >= 10:
        print(f"{Colors.GREEN}✅ {version.major}.{version.minor}.{version.micro}{Colors.END}")
        return True
    else:
        print(f"{Colors.RED}❌ {version.major}.{version.minor}.{version.micro} (需要3.10+){Colors.END}")
        return False
...
def check_optional():
    ...
    for package, description in OPTIONAL_PACKAGES.items():
        installed, version = check_package(package)
        if installed:
            print(f"  {Colors.GREEN}✅{Colors.END} {package}=={version} - {description}")
        else:
...
def check_env_file():
    ...
    if env_file.exists():
        ...
        with open(env_file, 'r') as f:
            ...
            missing = [var for var in critical_vars if var not in content or f"{var}=" in content or f"{var}=your" in content]
        ...
        for line in meminfo.split('\n'):
            if 'MemTotal' in line:
                ...
                if total_gb >= 2:
                    ...
                    print(f"{Colors.YELLOW}⚠️  {total_gb:.1f}GB (建议 > 2GB){Colors.END}")
                    return True
    except:
        pass
