...
class APITester:
    ...
    async def test_health(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"{Colors.GREEN}✅ 服务健康: {data.get('status', 'unknown')}{Colors.END}"
                )
                self.results.append(("Health", True))
                return True
            else:
                print(
                    f"{Colors.RED}❌ 健康检查失败: {response.status_code}{Colors.END}"
                )
    ...
    async def test_emotion(self):
        ...
        try:
            response = await self.client.post(
                "/api/v1/emotion/analyze", json={"text": "今天好开心啊！"}
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"{Colors.RED}❌ 情感分析失败: {response.status_code}{Colors.END}"
                )
    ...
    async def test_health_analysis(self):
        ...
        try:
            ...
            if response.status_code in [200, 422]:  # 422可能是缺少数据
                ...
                print(
                    f"{Colors.RED}❌ 健康分析失败: {response.status_code}{Colors.END}"
                )
    ...
    async def test_docs(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"{Colors.RED}❌ API文档访问失败: {response.status_code}{Colors.END}"
                )
