...
class TestEndToEndFlow:
    ...
    async def test_memory_context_consistency(self):
        ...
        # 查询相关话题
        response = await agent_v2.respond(
            user_id,
            "春天到了，想出去踏青"
        )
...
class TestPluginSystem:
    ...
    async def test_plugin_lifecycle(self):
        """插件生命周期测试"""
        from core.plugin.plugin_manager import plugin_manager, BasePlugin, PluginMetadata
...
class TestReportGeneration:
    ...
    async def test_health_report_generation(self):
        """健康报告生成测试"""
        from core.report.report_generator import report_generator, ReportFormat, ReportPeriod
        ...
        health_data = {
            ...
            "blood_glucose": [
                {"value": 5.5, "timestamp": datetime.utcnow()}
            ],
            "sleep": [
                {"duration_hours": 7.5, "efficiency": 85, "timestamp": datetime.utcnow()}
            ]
        }
