...
class TestAuthentication:
    ...
    def test_jwt_expiration(self):
        ...
        token = auth_manager.create_access_token(
            {"sub": "test_user"}, expires_delta=timedelta(seconds=0)
        )
...
class TestInputValidation:
    ...
    def test_sql_injection_detection(self, input_data):
        ...
        # 检测SQL注入模式
        has_sql = any(keyword in input_data.upper() for keyword in sql_keywords)
...
class TestErrorHandling:
    ...
    def test_error_messages_not_leak_info(self):
        ...
        # 生产环境不应暴露内部细节
        exc = RuoxiException(
            message="服务暂时不可用", details={"internal_id": 12345}  # 内部信息
        )
